...
exports.createDetails = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            ...
            mm.executeDML(`SELECT * FROM customer_master WHERE EMAIL = ? OR MOBILE_NO = ?`, [data.EMAIL, data.MOBILE_NO], supportKey, connection, (error, results) => {
                if (error) {
...
exports.updateDetails = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            const connection = mm.openConnection();
            mm.executeDML(`SELECT * FROM customer_master WHERE (EMAIL = ? OR MOBILE_NO = ?) AND ID!= ?`, [data.EMAIL, data.MOBILE_NO, data.CUSTOMER_ID, criteria.ID], supportKey, connection, (error, results) => {
                if (error) {
                    mm.rollbackConnection(connection);
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    ...
                    if (results.length > 0) {
                        ...
                        mm.executeDML(`UPDATE customer_master SET ${setData} CREATED_MODIFIED_DATE = '${systemDate}' WHERE ID = ${criteria.ID}`, recordData, supportKey, connection, (error, results) => {
                            if (error) {
                                ...
                                var ACTION_DETAILS = `User ${req.body.authData.data.UserData[0].NAME} has updated the customer details of  ${data.NAME}.`;
                                var logCategory = "customer"
