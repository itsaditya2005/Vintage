...
var couponCodeCountryMapping = "coupon_code_service_mapping";
...
function reqData(req) {
    var data = {
        ...
        SERVICE_CATELOG_ID: req.body.SERVICE_CATELOG_ID,
    }
    return data;
}
...
exports.validate = function () {
    return [
        ...
        body('SERVICE_CATELOG_ID').isInt().optional(),
...
exports.get = (req, res) => {
    ...
    try {
        if (IS_FILTER_WRONG == "0") {
            mm.executeQuery('select count(*) as cnt from ' + viewCouponCodeCountryMapping + ' where 1 ' + countCriteria, supportKey, (error, results1) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.send({
                        "code": 400,
                        "message": "Failed to get couponCodeCountryMapping count.",
                    });
                }
                else {
                    mm.executeQuery('select * from ' + viewCouponCodeCountryMapping + ' where 1 ' + criteria, supportKey, (error, results) => {
                        if (error) {
                            console.log(error);
                            logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                            res.send({
                                "code": 400,
                                "message": "Failed to get couponCodeCountryMapping information."
                            });
                        }
                        else {
                            res.send({
...
exports.create = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData('INSERT INTO ' + couponCodeCountryMapping + ' SET ?', data, supportKey, (error, results) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.send({
...
exports.update = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData(`UPDATE ` + couponCodeCountryMapping + ` SET ${setData} CREATED_MODIFIED_DATE = '${systemDate}' where ID = ${criteria.ID} `, recordData, supportKey, (error, results) => {
                if (error) {
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    console.log(error);
                    res.send({
