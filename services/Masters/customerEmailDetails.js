...
var customerEmailDetails = "customer_email_details";
var viewCustomerEmailDetails = "view_" + customerEmailDetails;
...
exports.validate = function () {
    return [
        ...
        body('CUSTOMER_ID').isInt(), body('EMAIL', ' parameter missing').exists(), body('MOBILE_NO', ' parameter missing').exists(), body('ID').optional(),
...
exports.get = (req, res) => {
    ...
    try {
        ...
        if (IS_FILTER_WRONG == "0") {
            mm.executeQuery('select count(*) as cnt from ' + viewCustomerEmailDetails + ' where 1 ' + countCriteria, supportKey, (error, results1) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.status(400).json({
                        "code": 400,
                        "message": "Failed to get customerEmailDetails count.",
                    });
                }
                else {
                    mm.executeQuery('select * from ' + viewCustomerEmailDetails + ' where 1 ' + criteria, supportKey, (error, results) => {
                        if (error) {
                            console.log(error);
                            logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                            res.status(400).json({
                                "code": 400,
                                "message": "Failed to get customerEmailDetails information."
                            });
                        }
                        else {
                            res.status(200).json({
...
exports.create = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData('INSERT INTO ' + customerEmailDetails + ' SET ?', data, supportKey, (error, results) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.status(400).json({
                        "code": 400,
                        "message": "Failed to save customerEmailDetails information..."
                    });
                }
                else {
                    res.status(200).json({
                        "code": 200,
                        "message": "CustomerEmailDetails information saved successfully...",
                    });
                }
            });
        } catch (error) {
...
exports.update = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData(`UPDATE ` + customerEmailDetails + ` SET ${setData} CREATED_MODIFIED_DATE = '${systemDate}' where ID = ${criteria.ID} `, recordData, supportKey, (error, results) => {
                if (error) {
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    console.log(error);
                    res.status(400).json({
                        "code": 400,
                        "message": "Failed to update customerEmailDetails information."
                    });
                }
                else {
                    res.status(200).json({
                        "code": 200,
                        "message": "CustomerEmailDetails information updated successfully...",
                    });
                }
            });
        } catch (error) {
            ...
            mm.executeQueryData(`SELECT * FROM customer_email_details WHERE EMAIL = ? OR MOBILE_NO = ?`, [data.EMAIL, data.MOBILE_NO], supportKey, (error, results) => {
                if (error) {
                    ...
                    if (results.length > 0) {
                        ...
                        mm.executeQueryData(`INSERT INTO customer_email_details (CUSTOMER_ID, SALUTATION, NAME, EMAIL, MOBILE_NO, STATUS, CLOUD_ID, W_CLOUD_ID, DEVICE_ID, LOGOUT_DATETIME, COUNTRY_CODE, CLIENT_ID, CREATED_DATE) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [data.CUSTOMER_ID, data.SALUTATION, data.NAME, data.EMAIL, data.MOBILE_NO, data.STATUS, data.CLOUD_ID, data.W_CLOUD_ID, data.DEVICE_ID, data.LOGOUT_DATETIME, data.COUNTRY_CODE, data.CLIENT_ID, mm.getSystemDate()], supportKey, (error, results1) => {
                            if (error) {
                                console.log(error);
                                logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                res.send({
                                ...
                                mm.executeQueryData('SELECT * FROM global_timeslots_settings ORDER BY ID DESC LIMIT 1', [], supportKey, (error, resultGet) => {
                                    if (error) {
                                        ...
                                        mm.executeQueryData('INSERT INTO global_time_slots_mapping (ORG_ID, MAPPING_FOR, MAPPING_ID, SLOT1_START_TIME, SLOT1_END_TIME, SLOT2_START_TIME, SLOT2_END_TIME, SLOT3_START_TIME, SLOT3_END_TIME,CLIENT_ID,CUSTOMER_DETAILS_ID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)', [0, "C", data.CUSTOMER_ID, resultGet[0].SLOT1_START_TIME, resultGet[0].SLOT1_END_TIME, resultGet[0].SLOT2_START_TIME, resultGet[0].SLOT2_END_TIME, resultGet[0].SLOT3_START_TIME, resultGet[0].SLOT3_END_TIME, 1, results1.insertId], supportKey, (error, resultsglobal) => {
                                            if (error) {
                                                console.log(error);
                                                logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                                res.send({
...
exports.updateDetails = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData(`SELECT * FROM customer_email_details WHERE (EMAIL = ? OR MOBILE_NO = ?) AND CUSTOMER_ID != ? AND ID!= ?`, [data.EMAIL, data.MOBILE_NO, data.CUSTOMER_ID, criteria.ID], supportKey, (error, results) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.send({
                    ...
                    if (results.length > 0) {
                        ...
                        mm.executeQueryData(`UPDATE customer_email_details SET ${setData} CREATED_MODIFIED_DATE = '${systemDate}' WHERE ID = ${criteria.ID}`, recordData, supportKey, (error, results) => {
                            if (error) {
                                logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                console.log(error);
                                res.send({
