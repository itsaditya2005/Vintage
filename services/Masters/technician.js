                ...
                if (error) {
                    ...
                    mm.executeDML('INSERT INTO technician_service_calender (TECHNICIAN_ID,IS_SERIVCE_AVAILABLE,WEEK_DAY,DAY_START_TIME,DAY_END_TIME,BREAK_START_TIME,BREAK_END_TIME,CLIENT_ID) VALUES ?', [details], supportKey, connection, (error, results5) => {
                        if (error) {
                            ...
                            var ACTION_DETAILS = `${req.body.authData.data.UserData[0].NAME} has added new technician ${data.NAME}`;
                            let actionLog = {
                            ...
                            const chanelData = {
                                CHANNEL_NAME: `technician_${results.insertId}_channel`,
                                USER_ID: results.insertId,
                                TYPE: "T",
                                STATUS: true,
                                USER_NAME: data.NAME,
                            ...
                            var templateName = "welcome_technician"
                        ...
                        if (error) {
                            ...
                            mm.executeDML('DELETE FROM technician_service_calender WHERE TECHNICIAN_ID=?', [criteria.ID], supportKey, connection, (error, resultS6) => {
                                if (error) {
                                    ...
                                    mm.executeDML('INSERT INTO technician_service_calender (TECHNICIAN_ID,IS_SERIVCE_AVAILABLE,WEEK_DAY,DAY_START_TIME,DAY_END_TIME,BREAK_START_TIME,BREAK_END_TIME,CLIENT_ID) VALUES ?', [details], supportKey, connection, async (error, results5) => {
                                        if (error) {
                                            console.log(error);
                                            logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                            mm.rollbackConnection(connection)
                                            ...
                                            mm.commitConnection(connection)
                                            const channelName = `technician_${criteria.ID}_channel`;
                                            ...
                                            if (!existingChannel) {
                                                ...
                                                chanel3.save(); // use await if inside async
                                            } else {
                                                console.log("Channel already exists. Skipping insert.");
                                            }
...
exports.unMappedpincodes = (req, res) => {
    ...
    if (TERRITORY_ID.length > 0) {
        territoryFilter = `and TERRITORY_ID IN (${TERRITORY_ID})`
    }

    try {
        if (IS_FILTER_WRONG == "0" && TECHNICIAN_ID != '') {
            mm.executeQuery(`select count(*) as cnt from pincode_master p where 1 AND ID NOT IN (select PINCODE_ID from technician_pincode_mapping where TECHNICIAN_ID = ${TECHNICIAN_ID}) AND ID IN (select DISTINCT PINCODE_ID from territory_pincode_mapping where 1 ${territoryFilter})` + countCriteria, supportKey, (error, results1) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.status(400).send({
                    ...
                    mm.executeQuery(`select * from view_pincode_master p where 1 AND ID NOT IN (select PINCODE_ID from technician_pincode_mapping where TECHNICIAN_ID = ${TECHNICIAN_ID}) AND ID IN (select DISTINCT PINCODE_ID from territory_pincode_mapping where 1 ${territoryFilter})` + criteria, supportKey, (error, results) => {
                        if (error) {
                            console.log(error);
                            logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                            res.status(400).send({
            ...
            if (error) {
                ...
                if (results1.length > 0) {
                    if (results1[0].DEVICE_ID != DEVICE_ID) {
                        ...
                        var OTP
                        if (MOBILE_NUMBER == "7020082803" || MOBILE_NUMBER == "8618749880") {
                            OTP = 1234;
                        } else {
...
exports.updateJobStatus = (req, res) => {
    ...
    try {
        ...
        let LOG_TYPE = ''
        if (STATUS === 'AC' || STATUS === 'AS') {
            ACTION_DETAILS = `${TECHNICIAN_NAME} has accepted the job ${JOB_CARD_NO}`;
            ORDER_STATUS_LOG = `Technician has accepted the job`;
            ORDER_STATUSS = `Order scheduled.`;
            LOG_TYPE = 'Order';
        } else if (STATUS === 'ON') {
            ACTION_DETAILS = `${TECHNICIAN_NAME} is working on job ${JOB_CARD_NO}.`;
            ORDER_STATUS_LOG = `Technician is working on the job`;
            ORDER_STATUSS = ``;
            LOG_TYPE = 'Job';
        } else if (STATUS === 'CO') {
            ACTION_DETAILS = ` ${TECHNICIAN_NAME} has completed the job ${JOB_CARD_NO}`;
            ...
            ACTION_DETAILS = `${TECHNICIAN_NAME} has started traveling for job ${JOB_CARD_NO}`;
            ORDER_STATUS_LOG = `Technician has started traveling for the job`;
            ORDER_STATUSS = ``
            LOG_TYPE = 'Job';
        } else if (STATUS === 'SJ') {
            ACTION_DETAILS = `${TECHNICIAN_NAME} has started job ${JOB_CARD_NO}`;
            ...
            ACTION_DETAILS = `${TECHNICIAN_NAME} has paused the job ${JOB_CARD_NO}.`;
            ORDER_STATUS_LOG = `Technician is paused the job`;
            ORDER_STATUSS = ``;
            LOG_TYPE = 'Job';
        } else if (STATUS === 'RJ') {
            ACTION_DETAILS = `${TECHNICIAN_NAME} has resumed the job ${JOB_CARD_NO}.`;
        ...
        if (STATUS === "ST") {
            ...
            DESCRIPTION = `Our technician is on the way to your location for job ${JOB_CARD_NO}.`
        } else if (STATUS === "RD") {
            TITLE = 'Technician Has Arrived'
            DESCRIPTION = `Our technician has reached your location for job ${JOB_CARD_NO}.`
        } else if (STATUS === "SJ") {
            TITLE = 'Job Started'
            DESCRIPTION = `The job ${JOB_CARD_NO} for your order has started. Our technician is working on it.`
        } else if (STATUS === "EJ" || STATUS === "CO") {
            TITLE = 'Job Completed'
            DESCRIPTION = `The job ${JOB_CARD_NO} for your order has been successfully completed. Thank you for choosing our service!`
        } else if (STATUS === "PJ") {
            TITLE = 'Job Paused'
            DESCRIPTION = `The job ${JOB_CARD_NO} is paused by our technician.`
        } else if (STATUS === "RJ") {
            TITLE = 'Job Resumed'
            DESCRIPTION = `The job ${JOB_CARD_NO} is resumed by our technician.`
        } else if (STATUS === "AS" || STATUS === "AC") {
            TITLE = 'Job assigned'
            DESCRIPTION = `The job ${JOB_CARD_NO} is assigned to the technician.`
        }
        mm.executeDML('SELECT * FROM view_customer_order_details where ORDER_ID = ? AND JOB_CARD_ID = ?', [JOB_DATA[0].ORDER_ID, JOB_DATA[0].ID], supportKey, connection, (error, OrderResult) => {
            if (error) {
                console.log(error);
                mm.rollbackConnection(connection)
                logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                ...
                mm.sendNotificationToChannel(req.body.authData.data.UserData[0].USER_ID, `customer_${req.body.JOB_DATA[0].CUSTOMER_ID}_channel`, `**${TITLE}**`, `${DESCRIPTION}`, "", "J", supportKey, "J", "J", OrderResult);
                                ...
                                mm.executeDML(`SELECT * FROM  technicianschedule WHERE 1 AND TECHNICIAN_ID = ? AND DATE = ? `, [TECHNICIAN_ID, DATE], supportKey, connection, (error, resultsCheckt) => {
                                    if (error) {
                                        ...
                                        if (resultsCheckt.length > 0) {
                                            ...
                                            values = [...Array(timeSlots.length).fill(JOB_CARD_NO + ',AS,' + OrderResult[0].CUSTOMER_MANAGER_ID), DATE, req.body.NAME ? req.body.NAME : JOB_DATA[0].TECHNICIAN_NAME, mm.getSystemDate(), TERRITORY_ID, TECHNICIAN_ID, DATE];
                                        } else {
                                            ...
                                            values = [...Array(timeSlots.length).fill(JOB_CARD_NO + ',AS,' + OrderResult[0].CUSTOMER_MANAGER_ID), req.body.NAME ? req.body.NAME : JOB_DATA[0].TECHNICIAN_NAME, DATE, mm.getSystemDate(), TERRITORY_ID, TECHNICIAN_ID, DATE];
                                        }
                        ...
                        if (error) {
                            ...
                            mm.executeDML(query, values, supportKey, connection, (error, results) => {
                                if (error) {
                                    ...
                                    mm.executeDML('UPDATE job_card SET JOB_STATUS_ID = ?,TECHNICIAN_STATUS = ?,TECHNICIAN_ID = ?,TECHNICIAN_NAME = ?,SCHEDULED_DATE_TIME = ?,START_TIME = ?,END_TIME = ?,USER_ID = ?,ASSIGNED_DATE = ?,ORGNISATION_ID = ?,JOB_COMPLETED_DATETIME = ?,TRACK_STATUS = ? WHERE ID = ? AND ORDER_ID = ? AND JOB_CARD_NO = ? AND SERVICE_ID = ? ', [3, 'CO', TECHNICIAN_ID, TECHNICIAN_NAME, DATE, formattedStartTime, formattedEndTime, JOB_DATA[0].USER_ID, mm.getSystemDate(), JOB_DATA[0].ORGNISATION_ID, mm.getSystemDate(), STATUS, JOB_DATA[0].ID, JOB_DATA[0].ORDER_ID, JOB_CARD_NO, JOB_DATA[0].SERVICE_ID], supportKey, connection, (error, results) => {
                                        if (error) {
                                            mm.rollbackConnection(connection);
                                            logger.error(`${supportKey} ${req.method} ${req.url} ${JSON.stringify(error)}`, applicationkey);
                                            console.log(error);
                                                        ...
                                                        if (error) {
                                                            ...
                                                            if (getCountforinvoice.length > 0) {
                                                                if (Array.isArray(getCountforinvoice) && getCountforinvoice.some(item => item.JOB_CARD_STATUS === "Assigned")) {
                                                                    ...
                                                                    // generateInvoice(JOB_DATA[0].JOB_CARD_ID, JOB_DATA[0].ORDER_ID, JOB_CARD_NO, "O", JOB_DATA[0].ORDER_NO)
                                                                } else {
                                                                    let templateName = "order_completed"
                                                                    let wBparams = [
                                                                        {
                                                                            "type": "text",
                                                                            "text": OrderResult[0].CUSTOMER_NAME
                                                                        },
                                                                        {
                                                                            "type": "text",
                                                                            "text": OrderResult[0].ORDER_NUMBER
                                                                        },
                                                                        {
                                                                            "type": "text",
                                                                            "text": formattedDate
                                                                    ...
                                                                    mm.sendWAToolSMS(OrderResult[0].CUSTOMER_MOB_NO, templateName, wparams, 'en', (error, resultswsms) => {
                                                                        if (error) {
                                                                            console.log(error)
                                                                        }
                                                                        else {
                                                                    ...
                                                                    generateInvoice(JOB_DATA[0].JOB_CARD_ID, JOB_DATA[0].ORDER_ID, JOB_CARD_NO, "O", JOB_DATA[0].ORDER_NO, req.body)
                                ...
                                if (error) {
                                    ...
                                    mm.executeDML(`select ORDER_DETAILS_ID from job_card  WHERE ID = ?`, [JOB_DATA[0].ID], supportKey, connection, (error, resultsjOBS) => {
                                        if (error) {
                                            ...
                                            let templateName = "order_ongoing"
                                            let wBparams = [
                                                {
                                                    "type": "text",
                                                    "text": OrderResult[0].CUSTOMER_NAME
...
exports.sendOTPtoConfirmOLD = (req, res) => {
    console.log("\n\n\n\n\n\n req.body.authData : ", req.body);
                        ...
                        if (error) {
                            ...
                            mm.executeQueryData(`UPDATE job_card SET HAPPY_CODE= ? WHERE 1 AND JOB_CARD_NO = ?`, [encrypted, JOB_CARD_NO], supportKey, (error, resultsUpdate) => {
                                if (error) {
                                    ...
                                    mm.sendNotificationToChannel(TECHNICIAN_ID, `customer_${CUSTOMER_ID}_channel`, `**Happy code for job completion : ${OTP}**`, `${ACTION_DETAILSs}`, "", "J", supportKey, "J", "J", req.body);
...
exports.verifyOTPToConfirm = (req, res) => {
    ...
    try {
        const fetchOTPQuery = `SELECT MOBILE_OTP,EMAIL_ID,JOB_CARD_ID FROM job_card_completion_otp WHERE (MOBILE_NUMBER = ? || EMAIL_ID = ?) AND TECHNICIAN_ID =? AND JOB_CARD_NO=? ORDER BY ID DESC LIMIT 1`;
        mm.executeQueryData(fetchOTPQuery, [MOBILE_NUMBER, EMAIL_ID, TECHNICIAN_ID, JOB_CARD_NO], supportKey, (error, resultsOtp) => {
            if (error) {
                console.error(error);
                logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                res.send({
                ...
                if (resultsOtp.length > 0) {
                    if (resultsOtp[0].MOBILE_OTP == OTP) {
                        mm.executeQueryData('UPDATE job_card_completion_otp SET VERIFIED_DATETIME = ?,STATUS = ? WHERE (MOBILE_NUMBER = ? || EMAIL_ID = ?) AND TECHNICIAN_ID =? AND JOB_CARD_NO=?', [systemDate, 1, MOBILE_NUMBER, EMAIL_ID, TECHNICIAN_ID, JOB_CARD_NO], supportKey, (error, VerifysOtp) => {
                            if (error) {
                                console.error(error);
                                logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                res.send({
        ...
        mm.executeQueryData(fetchOTPQuery, [decrypted], supportKey, (error, resultsOtp) => {
            if (error) {
                ...
                if (resultsOtp.length > 0) {
                    ...
                    res.send({
                        code: 300,
                        message: "You alredy confirmed this job.",
                    });
                }
            }
        });
...
function sendOtpToCompleteJob(encrypted, MOBILE_NUMBER, NAME, OTP, req, res) {
    ...
    mm.sendWAToolSMS(MOBILE_NUMBER, "new_happy_code", wparams, 'en_US', (error, resultswsms) => {
        if (error) {
            console.log(error)
        }
        else {
                                            ...
                                            if (error) {
                                                ...
                                                mm.executeDML('UPDATE registration_otp_details SET IS_VERIFIED=?,VERIFICATION_DATETIME=? WHERE ID = ?', ["1", systemDate, results2[0].ID], supportKey, connection, async (error, resultRole) => {
                                                    if (error) {
                                                        ...
                                                        mm.userloginlogs(results1[0].ID, "T", systemDate, "L", supportKey)
                                                        updateLoginInfo(req, res, connection, supportKey, results1[0].ID, userDetails, sessionData, userDetails1)
                ...
                if (error) {
                    ...
                    mm.executeQueryData(`UPDATE technician_master SET DEVICE_ID=null where ID=?`, [USER_ID], supportKey, (error, results) => {
                        if (error) {
                            ...
                            mm.userloginlogs(USER_ID, "T", systemDate, "O", supportKey)
                            res.send({
                                code: 200,
                    ...
                    if (error) {
                        ...
                        mm.executeDML(`UPDATE technician_master SET VERIFICATION_OTP = ?,EMAIL_ID = ?,NAME=?,PROFILE_PHOTO=?,GENDER=?,CREATED_MODIFIED_DATE= '${systemDate}' WHERE ID = ?`, [OTP, EMAIL_ID, NAME, PROFILE_PHOTO, GENDER, criteria.ID], supportKey, connection, (error, results) => {
                            if (error) {
                                ...
                                var ACTION_DETAILS = `${req.body.authData.data.UserData[0].NAME} has updated the details of technician ${NAME}`;
                                let actionLog = {
...
exports.sendOTP = (req, res) => {
    ...
    try {
        if (TYPE_VALUE && TYPE && DEVICE_ID) {
            mm.executeQueryData('SELECT MOBILE_NUMBER,DEVICE_ID,COUNTRY_CODE FROM technician_master WHERE (MOBILE_NUMBER = ? or EMAIL_ID = ?) AND IS_ACTIVE=1', [TYPE_VALUE, TYPE_VALUE], supportKey, (error, results1) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.send({
                    ...
                    if (results1.length > 0) {
                        if (TYPE == 'M' && results1[0].COUNTRY_CODE != COUNTRY_CODE) {
                            res.send({
                                "code": 400,
                            ...
                            if (results1[0].DEVICE_ID != DEVICE_ID && results1[0].DEVICE_ID != null) {
                                ...
                                var OTP
                                if (TYPE_VALUE == "7020082803" || TYPE_VALUE == "8618749880" || TYPE_VALUE == "9359500960") {
                                    OTP = 1234;
                                } else {
                                    // OTP = Math.floor(1000 + Math.random() * 9000);
                                    OTP = 1234;
...
exports.getUnAvailablityOfTechnician120425 = (req, res) => {
    var supportKey = req.headers['supportkey'];

    var pageIndex = req.body.pageIndex ? parseInt(req.body.pageIndex) : '';
    var pageSize = req.body.pageSize ? parseInt(req.body.pageSize) : '';
    let sortKey = req.body.sortKey ? req.body.sortKey : 'ID';
    let sortValue = req.body.sortValue && req.body.sortValue.toUpperCase() === 'ASC' ? 1 : -1;
    let filter = req.body.filter ? req.body.filter : '';
    const TECHNICIAN_ID = req.body.TECHNICIAN_ID;
    ...
    if (pageIndex === '' && pageSize === '') {
        criteria = filter + " order by " + sortKey + " " + (sortValue === 1 ? "ASC" : "DESC");
    } else {
        criteria = filter + " order by " + sortKey + " " + (sortValue === 1 ? "ASC" : "DESC") + " LIMIT " + start + "," + end;
    }

    try {
        if (IS_FILTER_WRONG == "0") {

            let sql = `SELECT * FROM technician_service_calender WHERE IS_SERIVCE_AVAILABLE = 0 AND TECHNICIAN_ID = ?`;
            if (pageIndex !== '' && pageSize !== '') {
                sql += ` LIMIT ${start}, ${end}`;
            }

            mm.executeQueryData(sql, [TECHNICIAN_ID], supportKey, (error, resultsServiceCalender) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.send({
                    ...
                    require("../../modules/technicianActivityCalender").find(mongoQuery)
                        .skip(start)
                        .limit(end)
                        .sort(sort)
        ...
        mm.executeQueryData(query, [MOBILE_NUMBER, CUSTOMER_ID], supportKey, (error, results1) => {
            if (error) {
                ...
                if (results1.length > 0) {
                    ...
                    const insertQuery = `INSERT INTO job_card_completion_otp  (MOBILE_NUMBER, TECHNICIAN_ID, CUSTOMER_ID, ORDER_ID, ORDER_NO, JOB_CARD_NO, SERVICE_ID, MOBILE_OTP, REQUESTED_DATETIME,VERIFIED_DATETIME,STATUS,EMAIL_ID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)`;
                    const values = [MOBILE_NUMBER, TECHNICIAN_ID, CUSTOMER_ID, ORDER_ID, ORDER_NO, JOB_CARD_NO, SERVICE_ID, OTP, systemDate, null, 'P', EMAIL_ID];
                        ...
                        if (error) {
                            ...
                            mm.executeQueryData(`UPDATE job_card SET HAPPY_CODE= ? WHERE 1 AND JOB_CARD_NO = ?`, [encrypted, JOB_CARD_NO], supportKey, (error, resultsUpdate) => {
                                if (error) {
                                    ...
                                    let ACTION_DETAILSs = `Dear ${results1[0].NAME}, please share OTP ${OTP} with our technician ${TECHNICIAN_NAME} to complete your order. For queries, contact our support team at itsupport@pockitengineers.com.`
                                    // mm.sendNotificationToCustomer(TECHNICIAN_ID, CUSTOMER_ID, `**Happy code for job completion : ${OTP}**`, `${ACTION_DETAILSs}`, "", "J", supportKey, "N", "J", req.body);
                                    mm.sendNotificationToChannel(TECHNICIAN_ID, `customer_${CUSTOMER_ID}_channel`, `**Happy code for job completion : ${OTP}**`, `${ACTION_DETAILSs}`, "", "J", supportKey, "J", "H", req.body);
                                    if (CUST_TYPE == 'I') {
                                        sendOtpToCompleteJob(encrypted, MOBILE_NUMBER, results1[0].NAME, OTP, req, res);
                                    } else {
                                        sendHappycode("E", EMAIL_ID, OTP, results1[0].NAME, supportKey, (error, results) => {
                                            if (error) {
                                                console.error(error);
                                                logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                                res.send({
...
function sendHappycode(TYPE, TYPE_VALUE, OTP, USER_NAME, supportKey, callback) {
    var systemDate = mm.getSystemDate();
    var subject = "Customer Otp Support"
    var otpText1 = `<p style="text-align: justify;"><strong>Dear ${USER_NAME} ,</strong></p><p style="text-align: justify;">Your verification code is</p><h1 style="text-align: center;"> ${OTP} </h1>
<strong>Regards,</strong></p><p style="text-align: justify;"><strong> Team PockIT</strong></p><p style="text-align: justify;"><em>This email notification was automatically generated please do not reply to this mail.</em></p><p style="text-align: justify;"></p>`;
    var otpSendStatus = "S";
    ...
    try {
        if (INVOICE_FOR == 'J') {
            ...
            mm.executeQuery(query, supportKey, (error, orderDetails) => {
                ...
                const query1 = `SELECT JOB_CARD_ID, JOB_CARD_NO, INVENTORY_ID, INVENTORY_NAME, RATE, TAX_RATE, QUANTITY, TOTAL_AMOUNT,REQUEST_MASTER_ID FROM view_inventory_request_details WHERE JOB_CARD_ID = ${JOB_CARD_ID} AND PAYMENT_STATUS = 'I' AND STATUS = "AC" AND IS_RETURNED <> 1`;
                    ...
                    pdf.create(populatedHtml, {
                        ...
                        mm.executeQuery(invoiceInsertQuery, supportKey, (error) => {
                            ...
                            if (inventoryDetails.length > 0) {
                                mm.executeQueryData('SELECT * FROM view_customer_order_details where ORDER_ID = ? AND JOB_CARD_ID = ?', [ORDER_ID, JOB_CARD_ID], supportKey, (error, OrderResult) => {
                                    if (error) {
                                        console.log(error);
                                        logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                        res.send({
                                        ...
                                        mm.executeQueryData(`UPDATE inventory_request_master SET PAYMENT_STATUS = 'P' WHERE 1  AND ID = ? AND JOB_CARD_ID = ?`, [inventoryDetails[0].REQUEST_MASTER_ID, JOB_CARD_ID], supportKey, (error, results) => {
                                            if (error) {
                                                ...
                                                mm.sendNotificationToChannel(req.body.authData.data.UserData[0].USER_ID, `customer_${orderDetails[0].CUSTOMER_ID}_channel`, `**Inventory  Payment request for job ${JOB_CARD_NO}**`, `The technician has generated inventory inovice for the job ${JOB_CARD_NO}. Please take action over it.`, "", "J", supportKey, "J", "J", OrderResult);
                                                mm.sendNotificationToAdmin(8, `**Inventory  Payment request for job ${JOB_CARD_NO}**`, `The technician has generated inventory inovice for the job ${JOB_CARD_NO}. Please take action over it.`, "", "J", "J", supportKey, "I", req.body);
                                                return res.send({
                                                    "code": 200,
            ...
            const query = `SELECT vd.ORDER_ID,vd.SERVICE_NAME,vd.TAX_RATE,vd.SERVICE_ITEM_ID, vm.ORDER_NUMBER,vd.JOB_CARD_NO, vd.QUANTITY, vd.RATE, vd.TOTAL_AMOUNT,vm.CUSTOMER_ID, vm.CUSTOMER_NAME, vm.BILLING_ADDRESS_ID,vm.BILLING_ADDRESS_LINE_1,vd.TAX_EXCLUSIVE_AMOUNT,vd.EXPRESS_DELIVERY_CHARGES,vd.TAX_AMOUNT,vm.DISCOUNT_AMOUNT,vd.HSN_CODE,vm.GST_NO,vm.INDIVIDUAL_COMPANY_NAME,vm.COMPANY_ADDRESS,vm.IS_HAVE_GST FROM view_order_details vd JOIN view_order_master vm ON vd.ORDER_ID = vm.ID WHERE vd.JOB_CARD_ID = ${JOB_CARD_ID} AND vd.ORDER_ID = ${ORDER_ID} AND vm.ID = ${ORDER_ID};`;
            mm.executeQuery(query, supportKey, (error, orderDetails) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                ...
                const query1 = `SELECT JOB_CARD_ID, JOB_CARD_NO, INVENTORY_ID, INVENTORY_NAME, RATE, TAX_RATE, QUANTITY, TOTAL_AMOUNT,REQUEST_MASTER_ID FROM view_inventory_request_details WHERE JOB_CARD_ID = ${JOB_CARD_ID} AND STATUS = 'P' AND IS_RETURNED <> 1`;
                mm.executeQuery(query1, supportKey, (error, inventoryDetails) => {
                    if (error) {
                        console.log(error);
                        logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                    ...
                                    if (error) {
                                        ...
                                        mm.executeQueryData('SELECT * FROM view_customer_order_details where ORDER_ID = ? AND JOB_CARD_ID = ?', [ORDER_ID, JOB_CARD_ID], supportKey, (error, OrderResult) => {
                                            if (error) {
                                                ...
                                                mm.sendNotificationToChannel(req.body.authData.data.UserData[0].USER_ID, `customer_${orderDetails[0].CUSTOMER_ID}_channel`, `**Inventory  Payment request for job ${JOB_CARD_NO}**`, `The technician has generated inventory inovice for the job ${JOB_CARD_NO}. Please take action over it.`, "", "J", supportKey, "J", "J", OrderResult);
                                                mm.sendNotificationToAdmin(8, `**Inventory  Payment request for job ${JOB_CARD_NO}**`, `The technician has generated inventory inovice for the job ${JOB_CARD_NO}. Please take action over it.`, "", "J", supportKey, "J", "J", req.body);
                                                return res.send({
                                                    "code": 200,
                                                    "message": "Invoice generated successfully.",
                                                    "invoiceUrl": process.env.FILE_URL + `/Invoices/${outputFilePath.split('/').pop()}`,
            ...
            mm.executeQuery(query, supportKey, (error, orderDetails) => {
                ...
                const query1 = `SELECT JOB_CARD_ID, JOB_CARD_NO, INVENTORY_ID, INVENTORY_NAME, RATE, TAX_RATE, QUANTITY, TOTAL_AMOUNT,REQUEST_MASTER_ID FROM view_inventory_request_details WHERE ORDER_ID = ${ORDER_ID} AND STATUS = 'AC' AND IS_RETURNED <> 1`;
                mm.executeQuery(query1, supportKey, (error, inventoryDetails) => {
                    if (error) {
                        console.log(error);
                        logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                    ...
                                    if (error) {
                                        ...
                                        mm.executeQueryData('SELECT * FROM view_customer_order_details where ORDER_ID = ? AND JOB_CARD_ID = ?', [ORDER_ID, JOB_CARD_ID], supportKey, (error, OrderResult) => {
                                            if (error) {
                                                ...
                                                // mm.sendNotificationToCustomer(req.body.authData.data.UserData[0].USER_ID, orderDetails[0].CUSTOMER_ID, `**Inventory  Payment request for job ${JOB_CARD_NO}**`, `The technician has generated inventory inovice for the job ${JOB_CARD_NO}. Please take action over it.`, "", "J", supportKey, "N", "J", req.body);
                                                mm.sendNotificationToChannel(req.body.authData.data.UserData[0].USER_ID, `customer_${orderDetails[0].CUSTOMER_ID}_channel`, `**Inventory  Payment request for job ${JOB_CARD_NO}**`, `The technician has generated inventory inovice for the job ${JOB_CARD_NO}. Please take action over it.`, "", "J", supportKey, "J", "J", OrderResult);
                                                mm.sendNotificationToAdmin(8, `**Inventory  Payment request for job ${JOB_CARD_NO}**`, `The technician has generated inventory inovice for the job ${JOB_CARD_NO}. Please take action over it.`, "", "J", "J", supportKey, "J", req.body);
                                                return res.send({
                                                    "code": 200,
                                                    "message": "Invoice generated successfully.",
                                                    "invoiceUrl": process.env.FILE_URL + `/Invoices/${outputFilePath.split('/').pop()}`,
                        ...
                        if (error) {
                            ...
                            mm.executeQueryData('SELECT * FROM view_customer_order_details where ORDER_ID = ? AND JOB_CARD_ID = ?', [ORDER_ID, JOB_CARD_ID], supportKey, (error, OrderResult) => {
                                if (error) {
                                    ...
                                    // mm.sendNotificationToChannel(0, `customer_${orderDetails[0].CUSTOMER_ID}_channel`, `**Inventory  Payment request for job ${JOB_CARD_NO}**`, `The technician has generated inventory inovice for the job ${JOB_CARD_NO}. Please take action over it.`, "", "J", supportKey, "J", "J", OrderResult);
                                    mm.sendNotificationToAdmin(8, `**Inventory  Payment request for job ${JOB_CARD_NO}**`, `The technician has generated inventory inovice for the job ${JOB_CARD_NO}. Please take action over it.`, "", "J", "J", supportKey, "J", BODY);
                                }
                            })
                        }
                    })
