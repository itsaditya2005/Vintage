...
exports.getTicketGroupwiseDetailedReport = (req, res) => {
    ...
    try {
        mm.executeQuery('select COUNT(*) as cnt from ' + viewTicketMaster + ' where 1 ' + countCriteria, supportKey, (error, results1) => {
            if (error) {
                ...
                mm.executeQuery('select ID,TICKET_GROUP_ID,TICKET_GROUP_VALUE,TICKET_NO,CREATOR_EMPLOYEE_NAME,CREATOR_EMPLOYEE_ID,QUESTION,IS_TAKEN_STATUS,TAKEN_BY_USER_ID,TICKET_TAKEN_EMPLOYEE,DATE,LAST_RESPONDED,PRIORITY,STATUS from ' + viewTicketMaster + ' where 1 ' + criteria, supportKey, (error, results) => {
                    if (error) {
                        console.log(error);
                        logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                        res.send({
