...
exports.createold = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData(`SELECT * FROM customer_email_details WHERE EMAIL = ? OR MOBILE_NO = ?`, [data.EMAIL, data.MOBILE_NO], supportKey, (error, results) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.send({
                    ...
                    if (results.length > 0) {
                        if (results[0].EMAIL === data.EMAIL && results[0].MOBILE_NO === data.MOBILE_NO) {
                            res.send({
                                "code": 300,
                        ...
                        } else if (results[0].MOBILE_NO === data.MOBILE_NO) {
                            res.send({
                                "code": 300,
                        ...
                        mm.executeQueryData('INSERT INTO ' + customerMaster + ' SET ?', data, supportKey, (error, results) => {
                            if (error) {
                                ...
                                mm.executeQueryData(`INSERT INTO customer_email_details (CUSTOMER_ID,SALUTATION,NAME,EMAIL, MOBILE_NO, STATUS, CLOUD_ID, W_CLOUD_ID, DEVICE_ID, LOGOUT_DATETIME, COUNTRY_CODE, CLIENT_ID,CREATED_DATE) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)`, [results.insertId, data.SALUTATION, data.NAME, data.EMAIL, data.MOBILE_NO, '1', null, null, null, null, data.COUNTRY_CODE, data.CLIENT_ID, mm.getSystemDate()], supportKey, (error, results1) => {
                                    if (error) {
                                        console.log(error);
                                        logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                        res.send({
                                        ...
                                        if (data.CUSTOMER_TYPE == "B") {
                                            mm.executeQueryData('SELECT * FROM global_timeslots_settings ORDER BY ID DESC LIMIT 1', [], supportKey, (error, resultGet) => {
                                                if (error) {
                                                    ...
                                                    mm.executeQueryData('INSERT INTO global_time_slots_mapping (ORG_ID, MAPPING_FOR, MAPPING_ID, SLOT1_START_TIME, SLOT1_END_TIME, SLOT2_START_TIME, SLOT2_END_TIME, SLOT3_START_TIME, SLOT3_END_TIME,CLIENT_ID,CUSTOMER_DETAILS_ID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)', [0, "C", results.insertId, resultGet[0].SLOT1_START_TIME, resultGet[0].SLOT1_END_TIME, resultGet[0].SLOT2_START_TIME, resultGet[0].SLOT2_END_TIME, resultGet[0].SLOT3_START_TIME, resultGet[0].SLOT3_END_TIME, 1, results1.insertId], supportKey, (error, resultsglobal) => {
                                                        if (error) {
                                                            console.log(error);
                                                            logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                                            res.send({
...
exports.updateold = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData(`SELECT * FROM customer_email_details WHERE (EMAIL = ? OR MOBILE_NO = ?) AND CUSTOMER_ID != ?`, [data.EMAIL, data.MOBILE_NO, criteria.ID], supportKey, (error, results) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.send({
                                ...
                                if (data.CUSTOMER_TYPE == "B") {
                                    mm.executeQueryData('SELECT * FROM global_timeslots_settings ORDER BY ID DESC LIMIT 1', [], supportKey, (error, resultGet) => {
                                        if (error) {
                                            ...
                                            mm.executeQueryData('SELECT * FROM global_time_slots_mapping WHERE MAPPING_FOR = "C" AND MAPPING_ID = ?', [criteria.ID], supportKey, (error, resultGetSlot) => {
                                                if (error) {
                                                    console.log(error);
                                                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                                    res.send({
                                                    ...
                                                    if (resultGetSlot.length > 0) {
                                                        ...
                                                        mm.executeQueryData('INSERT INTO global_time_slots_mapping (ORG_ID, MAPPING_FOR, MAPPING_ID, SLOT1_START_TIME, SLOT1_END_TIME, SLOT2_START_TIME, SLOT2_END_TIME, SLOT3_START_TIME, SLOT3_END_TIME,CLIENT_ID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [0, "C", criteria.ID, resultGet[0].SLOT1_START_TIME, resultGet[0].SLOT1_END_TIME, resultGet[0].SLOT2_START_TIME, resultGet[0].SLOT2_END_TIME, resultGet[0].SLOT3_START_TIME, resultGet[0].SLOT3_END_TIME, 1], supportKey, (error, resultsglobal) => {
                                                            if (error) {
                                                                console.log(error);
                                                                logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                                                res.send({
                                    ...
                                    var ACTION_DETAILS = `User ${req.body.authData.data.UserData[0].NAME} has updated customer details of  ${data.NAME}.`;
                                    var logCategory = "customer"
                ...
                if (resultsUser.length > 0) {
                    mm.executeQueryData(`UPDATE ` + customerMaster + ` SET PASSWORD =?, CREATED_MODIFIED_DATE =? where ID = ? `, [NEW_PASSWORD, systemDate, ID], supportKey, (error, results) => {
                        if (error) {
                            ...
                            var ACTION_DETAILS = `${req.body.authData.data.UserData[0].NAME} has changed the password ${resultsUser.NAME}.`;
                            var logCategory = "customer"
        ...
        mm.executeDML("SELECT * FROM customer_email_master WHERE MOBILE_NO= ? or EMAIL= ?", [req.body.MOBILE_NO, req.body.EMAIL_ID], supportKey, connection, (err, custResult) => {
            if (err) {
                ...
                if (custResult.length > 0) {
                    if (custResult[0].EMAIL === req.body.EMAIL_ID && custResult[0].MOBILE_NO === req.body.MOBILE_NO) {
                        res.send({
                            "code": 300,
                            "message": "Email ID and mobile number already exist."
                        });
...
function sendOtpOLD(TYPE, TYPE_VALUE, subject, body, OTP, USER_NAME, supportKey, callback) {
    var systemDate = mm.getSystemDate();
    console.log("TYPE : ", TYPE, "TYPE_VALUE :", TYPE_VALUE);
    var subject = "Customer Otp Support"
    var otpText1
    if (TYPE == "M") {
        otpText1 = `Your OTP is ${OTP}. This code is valid for the next [5 minutes]. Please do not share it with anyone.`
    } else {
                                                ...
                                                mm.executeDML(`INSERT into customer_master (NAME,EMAIL,MOBILE_NO,CLIENT_ID,ACCOUNT_STATUS,IS_SPECIAL_CATALOGUE,REGISTRATION_DATE,CLOUD_ID,W_CLOUD_ID,COUNTRY_CODE,CUSTOMER_TYPE,CUSTOMER_CATEGORY_ID,CUSTOMER_DETAILS_ID,IS_PARENT) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?) `, [CUSTOMER_NAME, CUSTOMER_EMAIL_ID, CUSTOMER_MOBILE_NO, 1, ACCOUNT_STATUS, IS_SPECIAL_CATALOGUE, systemDate, CLOUD_ID, W_CLOUD_ID, COUNTRY_CODE, CUSTOMER_TYPE, CUSTOMER_CATEGORY_ID, resultCustomer1.insertId, 1], supportKey, connection, async (error, resultCustomer2) => {
                                                    ...
                                                    if (error) {
                                                        ...
                                                        var userDetails = [{
                                                            USER_ID: resultCustomer2.insertId,
                                                            USER_NAME: CUSTOMER_NAME,
                                                            MOBILE_NUMBER: CUSTOMER_MOBILE_NO,
                                                            CLIENT_ID: 1,
                                                            isPresent: 1,
...
function sendOtp(TYPE, TYPE_VALUE, subject, body, OTP, USER_NAME, supportKey, callback) {
    ...
    if (TYPE == "M") {
        ...
        otpText1 = `<p style="text-align: justify;"><strong>Dear Customer,</strong></p><p style="text-align: justify;">Your one-time password (OTP) for email verification is</p><h1 style="text-align: center;"> ${OTP} </h1><p style="text-align: justify;">Please do not share this one time password with anyone.<br />In case you need any further clarification for the same, <br />please do get in touch immediately with itsupport@pockitengineers.com.</p><p style="text-align: justify;"><strong>Regards,</strong></p><p style="text-align: justify;"><strong> Team PockIT</strong></p><p style="text-align: justify;"><em>This email notification was automatically generated, please do not reply to this email.</em></p><p style="text-align: justify;"><em>If you have any suggestions or feedback, please provide them through our official website: https://pockitapp.pockitengineers.com/</em></p>`;
    }
    var otpSendStatus = "S";
    mm.executeQueryData(`INSERT INTO registration_otp_details(TYPE,TYPE_VALUE,OTP,OTP_MESSAGE,REQUESTED_DATETIME,CLIENT_ID,STATUS,IS_VERIFIED,OTP_TYPE) values(?,?,?,?,?,?,?,?,?)`, [TYPE, TYPE_VALUE, OTP, otpText1, systemDate, 1, 'S', '0', 'C'], supportKey, (error, insertOtpDetails) => {
        if (error) {
...
exports.create = (req, res) => {
    ...
    // data.PASSWORD = generateRandomAlphanumeric();
    data.PASSWORD = data.CUSTOMER_TYPE == "B" ? md5(data.PASSWORD) : null;
    var systemDate = mm.getSystemDate();
    data.REGISTRATION_DATE = systemDate;
    const errors = validationResult(req);
    var supportKey = req.headers['supportkey'];
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            const connection = mm.openConnection();
            mm.executeDML(`SELECT * FROM customer_email_master WHERE EMAIL = ? OR MOBILE_NO = ?`, [data.EMAIL, data.MOBILE_NO], supportKey, connection, (error, results) => {
                if (error) {
                    mm.rollbackConnection(connection);
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
...
exports.update = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            const connection = mm.openConnection();
            mm.executeDML(`SELECT * FROM customer_email_master WHERE (EMAIL = ? OR MOBILE_NO = ?) AND ID != ?`, [data.EMAIL, data.MOBILE_NO, CUSTOMER_EMAIL_ID], supportKey, connection, (error, results) => {
                if (error) {
                    mm.rollbackConnection(connection)
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
...
exports.getCustomerDetails = (req, res) => {
    ...
    try {
        if (IS_FILTER_WRONG == "0") {
            mm.executeQuery('select count(*) as cnt from customer_email_master where 1 ' + countCriteria, supportKey, (error, results1) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.send({
                    ...
                    mm.executeQuery('select *,(SELECT ID FROM customer_master WHERE customer_master.CUSTOMER_DETAILS_ID = customer_email_master.ID AND customer_master.IS_PARENT = 1) AS CUSTOMER_MASTER_ID  from customer_email_master where 1 ' + criteria, supportKey, (error, results) => {
                        if (error) {
                            console.log(error);
                            logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                            res.send({
