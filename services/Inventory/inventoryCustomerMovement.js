...
exports.getAll = (req, res) => {
    ...
    try {
        if (IS_FILTER_WRONG == "0") {
            mm.executeQuery('select count(*) as cnt from ' + viewinventoryCustomerMovement + ' where 1 ' + countCriteria, supportKey, (error, results1) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.status(400).send({
                        "message": "Failed to get inventoryCustomerMovement count.",
                    });
                }
                else {

                    mm.executeQuery('select * from ' + viewinventoryCustomerMovement + ' where 1 ' + criteria, supportKey, (error, results) => {
                        if (error) {
                            console.log(error);
                            logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                            res.status(400).send({
                                "message": "Failed to get inventoryCustomerMovement information."
                            });
                        }
                        else {
                            res.status(200).send({
...
exports.get = (req, res) => {
    ...
    try {
        if (IS_FILTER_WRONG == "0") {
            mm.executeQueryData('select count(*) as cnt from ' + viewinventoryCustomerMovement + ' where 1 AND ID=? ' + countCriteria, ID, supportKey, (error, results1) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.status(400).send({
                    ...
                    mm.executeQueryData('select * from ' + viewinventoryCustomerMovement + ' where 1 AND ID=?' + criteria, ID, supportKey, (error, results) => {
                        if (error) {
                            console.log(error);
...
exports.create = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData('INSERT INTO ' + inventoryCustomerMovement + ' SET ?', data, supportKey, (error, results) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.status(400).send({
                        "message": "Failed to save inventoryCustomerMovement information..."
                    });
                }
                else {
                    res.status(200).send({
                        "message": "inventoryCustomerMovement information saved successfully...",
                    });
                }
            });
        } catch (error) {
...
exports.update = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData(`UPDATE ` + inventoryCustomerMovement + ` SET ${setData} CREATED_MODIFIED_DATE = '${systemDate}' where ID = ${criteria.ID} `, recordData, supportKey, (error, results) => {
                if (error) {
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    console.log(error);
                    res.status(400).send({
                        "message": "Failed to update inventoryCustomerMovement information."
                    });
                }
                else {
                    res.status(200).send({
                        "message": "inventoryCustomerMovement information updated successfully...",
                    });
                }
            });
        } catch (error) {
...
exports.createMovement = (req, res) => {
    ...
    try {
        ...
        if (ITEM_IDS.length > 0) {
            ...
            mm.executeDML(`UPDATE inventory_account_transaction SET IS_RETURNED=1 WHERE ID IN (?)`, [itemIds], supportKey, connection, (error, results8) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    ...
                    mm.executeDML(`UPDATE inventory_request_details SET IS_RETURNED=1 WHERE ID IN (?)`, [inventoryRequestIDS], supportKey, connection, (error, results8) => {
                        if (error) {
                            console.log(error);
                            logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                            mm.rollbackConnection(connection);
                            ...
                            mm.executeDML('INSERT INTO inventory_customer_movement (MOVEMENT_NUMBER,CUSTOMER_ID,TECHNICIAN_ID,DATE,USER_ID,USER_NAME,CUSTOMER_NAME,TECHNICIAN_NAME,MOVEMENT_TYPE,CLIENT_ID,TRANSFER_MODE) VALUES (?,?,?,?,?,?,?,?,?,?,?)', [MOVEMENT_NUMBER, CUSTOMER_ID, TECHNICIAN_ID, DATE, USER_ID, USER_NAME, CUSTOMER_NAME, TECHNICIAN_NAME, MOVEMENT_TYPE, CLIENT_ID, TRANSFER_MODE], supportKey, connection, (error, results1) => {
                                if (error) {
                                    console.log(error);
                                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                    mm.rollbackConnection(connection);
                                    ...
                                    async.eachSeries(INVENTORY_DETAILS, function iteratorOverElems(movementData, callback) {
                                        ...
                                        if (movementData.INVENTORY_ID != null && movementData.INVENTORY_ID != undefined && movementData.INVENTORY_ID != '') {
                                            mm.executeDML('INSERT INTO inventory_customer_movement_details (MOVEMENT_ID,INVENTORY_ID,IS_VARIENT,PARENT_ID,VARIANT_NAME,INVENTORY_NAME,QUANTITY,UNIT_ID,UNIT_NAME,QUANTITY_PER_UNIT,INVENTORY_CAT_ID,INVENTORY_CAT_NAME,INVENTROY_SUB_CAT_ID,INVENTROY_SUB_CAT_NAME,MOVEMENT_TYPE,CLIENT_ID,SERIAL_NO,BATCH_NO,JOB_CARD_ID) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [results1.insertId, movementData.INVENTORY_ID, movementData.IS_VERIENT, movementData.PARENT_ID, movementData.VARIANT_NAME, movementData.INVENTORY_NAME, movementData.QUANTITY, movementData.UNIT_ID, movementData.UNIT_NAME, movementData.QUANTITY_PER_UNIT, movementData.INVENTORY_CAT_ID, movementData.INVENTORY_CAT_NAME, movementData.INVENTROY_SUB_CAT_ID, movementData.INVENTROY_SUB_CAT_NAME, MOVEMENT_TYPE, CLIENT_ID, movementData.SERIAL_NO, movementData.BATCH_NO, JOB_CARD_ID], supportKey, connection, (error, results2) => {
                                                if (error) {
                                                    console.log(error);
                                                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                                                    callback(error)
                                                    ...
                                                    mm.executeDML('select TRANSACTION_ID FROM inventory_account_transaction WHERE GATEWAY_TYPE="T"  ORDER BY ID DESC LIMIT 1 ', [], supportKey, connection, (error, results3) => {
                                                        if (error) {
                                                            ...
                                                            var data = [[TRANSACTION_ID, mm.getSystemDate(), "D", movementData.INVENTORY_TRACKING_TYPE, 0, 0, 0, 0, 0, movementData.JOB_CARD_ID, "M", movementData.BATCH_NO, movementData.SERIAL_NO, movementData.INVENTORY_ID, 0, movementData.QUANTITY, ACTION_LOG, CLIENT_ID, movementData.UNIT_ID, movementData.UNIT_NAME, movementData.IS_VERIENT, movementData.PARENT_ID, movementData.QUANTITY_PER_UNIT],
                                                            ...
                                                            ACTION_LOG = `${USER_NAME} has transferred the inventory to technician ${TECHNICIAN_NAME}`;
...
exports.detailedList = (req, res) => {
    ...
    try {
        if (IS_FILTER_WRONG == "0") {
            mm.executeQuery('select count(*) as cnt from ' + viewinventoryCustomerMovement + ' IM  where 1 ' + countCriteria, supportKey, (error, results1) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.status(400).send({
                    ...
                    mm.executeQuery('select *,(select count(*) from view_inventory_customer_movement_details where MOVEMENT_ID=IM.ID) as INVENTORY_COUNT from ' + viewinventoryCustomerMovement + ' IM where 1 ' + criteria, supportKey, (error, results) => {
                        if (error) {
                            console.log(error);
