...
exports.getAll = (req, res) => {
    ...
    try {
        if (IS_FILTER_WRONG == 0) {
            mm.executeQuery('select count(*) as cnt from ' + viewinventoryTechnicianMovementDetails + ' where 1 ' + countCriteria, supportKey, (error, results1) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.status(400).send({
                        "message": "Failed to get inventoryTechnicianMovementDetails count.",
                    });
                }
                else {
                    mm.executeQuery('select * from ' + viewinventoryTechnicianMovementDetails + ' where 1 ' + criteria, supportKey, (error, results) => {
                        if (error) {
                            console.log(error);
                            logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                            res.status(400).send({
                                "message": "Failed to get inventoryTechnicianMovementDetails information."
                            });
                        }
                        else {
                            res.status(200).send({
...
exports.get = (req, res) => {
    ...
    try {
        if (IS_FILTER_WRONG == 0) {
            mm.executeQueryData('select count(*) as cnt from ' + viewinventoryTechnicianMovementDetails + ' where 1 AND ID=?' + countCriteria, ID, supportKey, (error, results1) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.status(400).send({
                    ...
                    mm.executeQueryData('select * from ' + viewinventoryTechnicianMovementDetails + ' where 1 AND ID=?' + criteria, ID, supportKey, (error, results) => {
                        if (error) {
                            console.log(error);
                            logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                            res.status(400).send({
...
exports.movementDetails = (req, res) => {
    ...
    try {
        if (IS_FILTER_WRONG == 0) {
            mm.executeQueryData('select count(*) as cnt from ' + viewinventoryTechnicianMovementDetails + ' where 1 AND MOVEMENT_ID=?' + countCriteria, ID, supportKey, (error, results1) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.status(400).send({
                    ...
                    mm.executeQueryData('select * from ' + viewinventoryTechnicianMovementDetails + ' where 1 AND MOVEMENT_ID=?' + criteria, ID, supportKey, (error, results) => {
                        if (error) {
                            console.log(error);
                            logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                            res.status(400).send({
...
exports.create = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData('INSERT INTO ' + inventoryTechnicianMovementDetails + ' SET ?', data, supportKey, (error, results) => {
                if (error) {
                    console.log(error);
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    res.status(400).send({
                        "message": "Failed to save inventoryTechnicianMovementDetails information..."
                    });
                }
                else {
                    res.status(200).send({
                        "message": "inventoryTechnicianMovementDetails information saved successfully...",
                    });
                }
            });
        } catch (error) {
...
exports.update = (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            mm.executeQueryData(`UPDATE ` + inventoryTechnicianMovementDetails + ` SET ${setData} CREATED_MODIFIED_DATE = '${systemDate}' where ID = ${criteria.ID} `, recordData, supportKey, (error, results) => {
                if (error) {
                    logger.error(supportKey + ' ' + req.method + " " + req.url + ' ' + JSON.stringify(error), applicationkey);
                    console.log(error);
                    res.status(400).send({
                        "message": "Failed to update inventoryTechnicianMovementDetails information."
                    });
                }
                else {
                    res.status(200).send({
                        "message": "inventoryTechnicianMovementDetails information updated successfully...",
                    });
                }
            });
        } catch (error) {
